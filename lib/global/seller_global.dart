...
class SellerGlobalHandler {
  static Future setToken(String token) async {
    await SharedPreferences.getInstance().then((prefs) {
      prefs.setString("seller_token", token);
      prefs.setBool("seller_is_loggedin", true);
    });
  }
  ...
  static logout() async {
    await SharedPreferences.getInstance().then((prefs) {
      prefs.setString("seller_token", "");
      prefs.setBool("seller_is_loggedin", false);
      exit(0);
    });
  }
  ...
  static Future<bool> checkLogedin() async {
    ...
    var isLogedin = prefs.getBool("seller_is_loggedin");
